...
(function ($) {
    ...
    //                     // callbacks: {
    //                     //     close: function () {
    //                     //         // Always set a cookie so it won’t keep showing
    //                     //         setCookie(cookieName, 'shown', expireDays);

    //                     //         // If user checks "don't show again", extend to 30 days or more
    //                     //         if ($('#newsletter-off').is(':checked')) {
    //                     //             setCookie(cookieName, 'shown', expireDays); // or longer if you prefer
    //                     //         }
    //                     //     }
    //                     // }
    //                     // callbacks: {
    ...
    //                     //         if ($('#newsletter-off').is(':checked')) {
    //                     //             // user does not want it at all → 30 days
    //                     //             setCookie(cookieName, 'shown', expireDays);
    //                     //         } else {
    //                     //             // normal close → show again after 12 hours
    //                     //             setCookie(cookieName, 'shown', expireDays);
    //                     //         }
    ...
    if (typeof $.fn.magnificPopup === 'function') {
        if ($('#subscribe-popup').length > 0) {
            ...
            if (saved) {
                ...
                // If not expired → STOP and don't show popup
                if (now < data.expires) {
                    // console.log("Popup skipped (not expired).");
                    return;
                }
            }

            // Otherwise show popup after delay
            setTimeout(function () {
                $.magnificPopup.open({
                    showCloseBtn: false,
                    ...
                    callbacks: {
                        ...
                        close: function () {
                            console.log("Popup closed → saving to localStorage");

                            let expiresAt = Date.now() + (expireHours * 60 * 60 * 1000);
    ...
    function setCookie(cname, cvalue, exdays) {
        const maxAge = exdays * 24 * 60 * 60; // convert days to seconds
    ...
    function setupSwiper() {
        ...
        // Helper: safely add/merge an event handler to the top-level sliderOptions.on
        function addSwiperEvent(sliderOptions, eventName, callback) {
            if (!sliderOptions.on) sliderOptions.on = {};
            if (!sliderOptions.on[eventName]) {
                sliderOptions.on[eventName] = callback;
            } else {
                var old = sliderOptions.on[eventName];
                sliderOptions.on[eventName] = function () {
                    try { old.apply(this, arguments); } catch (e) { console.error(e); }
                    try { callback.apply(this, arguments); } catch (e) { console.error(e); }
                };
            }
        }

        // Helper: safely add/merge an event handler to a nested slider object (e.g., sliderOptions.thumbs.swiper)
        function addNestedEvent(nestedObj, eventName, callback) {
            if (!nestedObj.on) nestedObj.on = {};
            if (!nestedObj.on[eventName]) {
                nestedObj.on[eventName] = callback;
            } else {
                var old = nestedObj.on[eventName];
                nestedObj.on[eventName] = function () {
                    try { old.apply(this, arguments); } catch (e) { console.error(e); }
                    try { callback.apply(this, arguments); } catch (e) { console.error(e); }
                };
            }
        ...
        swipers.forEach(function (swiperItem) {
            ...
            if (typeof (sliderOptions) !== 'undefined' && sliderOptions !== null) {
                ...
                if (changeOnClick != '' && changeOnClick != undefined && changeOnClick == '1') {
                    addSwiperEvent(sliderOptions, 'click', function () {
                ...
                if (sliderOptions['thumbs'] != '' && sliderOptions['thumbs'] != undefined) {
                    ...
                    if (mdThumbDirection != '' && mdThumbDirection != undefined) {
                        var thumbSwiperObj = sliderOptions['thumbs']['swiper'] = sliderOptions['thumbs']['swiper'] || {};

                        var thumbDirection = (thumbSwiperObj['direction'] != '' && thumbSwiperObj['direction'] != undefined) ? thumbSwiperObj['direction'] : mdThumbDirection;

                        addNestedEvent(thumbSwiperObj, 'init', function () {
                        ...
                        addNestedEvent(thumbSwiperObj, 'resize', function () {
                            if (getWindowWidth() <= sliderBreakPoint) {
                                this.changeDirection(mdThumbDirection);
                            } else {
                                this.changeDirection(thumbDirection);
                        ...
                        addNestedEvent(thumbSwiperObj, 'click', function () {
                ...
                // Original anime/fancy init + slideChange logic — add as merged handlers (so they don't overwrite other handlers)
                addSwiperEvent(sliderOptions, 'init', function () {
                ...
                addSwiperEvent(sliderOptions, 'slideChange', function () {
                    // Get active slide
                    let slides = this.slides;
                    let activeIndex = this.activeIndex,
                        current_slide = this.slides[activeIndex],
                ...
                // If swiper has number navigation / progress / gallery — merge these handlers instead of overwriting
                ...
                if (thumbClick && sliderOptions.hasOwnProperty('thumbs')) {
                    // Ensure thumbs.swiper exists
                    sliderOptions['thumbs']['swiper'] = sliderOptions['thumbs']['swiper'] || {};
                    addNestedEvent(sliderOptions['thumbs']['swiper'], 'click', function (swiper) {
                ...
                // Finally initialize Swiper (after merging all handlers)
                if (typeof Swiper === 'function') {
                    _this.imagesLoaded(function () {
                        try {
                            var swiperObj = new Swiper(swiperItem, sliderOptions);
                            // keep reference if you use swiperObjs elsewhere
                            if (window.swiperObjs && Array.isArray(window.swiperObjs)) {
                                window.swiperObjs.push(swiperObj);
                            } else {
                                window.swiperObjs = [swiperObj];
                            }
                        } catch (e) {
